...
fn golden_json_itself_is_sane() {
    ...
    assert_eq!(rows.len(), 6, "pll.c emits six rates");
    ...
    // K is never zero on the wire: setPLL sends 1 instead (video.cpp:294).
    for r in &rows {
...
fn fpix_is_not_silently_rounded() {
    ...
    assert_eq!(solve_pll(r.fout).f_pix_mhz.to_bits(), r.fpix.to_bits());
}
